...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat& _img, const cv::Mat& _img1) {
                    ...
                    for (int i = 0; i < cur_img.rows; ++i) {
                        for (int j = 0; j < cur_img.cols; ++j) {
                            ...
                            if (!_img1.empty() && stereo_cam) {
                                result = static_cast<int>(rightImg.at<uchar>(i, j) * scale);
                                rightImg.at<uchar>(i, j) = (result > 255) ? 255 : static_cast<uchar>(result);
                            }
#endif
                        }
                    }
    ...
    if (prev_pts.size() > 0) {
        ...
        if (hasPrediction) {
            ...
            cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse = cv::cuda::SparsePyrLKOpticalFlow::create(
                                                                           cv::Size(OPTICAL_WINSIZE, OPTICAL_WINSIZE), 1, 30, true);
            ...
            if (succ_num < 10) {
                cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse = cv::cuda::SparsePyrLKOpticalFlow::create(
                                                                               cv::Size(OPTICAL_WINSIZE, OPTICAL_WINSIZE), OPTICAL_LAYER_NUM, 30, false);
                d_pyrLK_sparse->calc(prev_gpu_img, cur_gpu_img, prev_gpu_pts, cur_gpu_pts, gpu_status);
...

    {
        ...
        if (n_max_cnt > 0) {
            ...
            cv::Ptr<cv::cuda::CornersDetector> detector = cv::cuda::createGoodFeaturesToTrackDetector(cur_gpu_img.type(), MAX_CNT - 100 - cur_pts.size(), 0.01, MIN_DIST);
            // LOG_OUT << "new gpu points: "<< MAX_CNT - cur_pts.size()<<endl;
            detector->detect(cur_gpu_img, d_prevPts, gpu_mask);
            // LOG_OUT << "d_prevPts size: "<< d_prevPts.size()<<std::endl;
            if (!d_prevPts.empty())
        ...
        TicToc t_gg;
        cv::cuda::GpuMat gpu_mask(mask);;
        cv::Ptr<cv::cuda::CornersDetector> detector = cv::cuda::createGoodFeaturesToTrackDetector(cur_gpu_img.type(), 100, 0.001, 50);
        detector->detect(cur_gpu_img, d_prevPts, gpu_mask);
        if (!d_prevPts.empty())
            n_pts = cv::Mat_<cv::Point2f>(cv::Mat(d_prevPts));
        else
    ...
    if (!_img1.empty() && stereo_cam) {
        ...
        if (!cur_pts.empty()) {
...

            {
                ...
                if (FLOW_BACK) {
                    cv::cuda::GpuMat reverseLeft_gpu_Pts = cur_gpu_pts;
