...
#include "gnss_ceres_factor.h"
#include "common_function.h"

bool SppPseudorangeCeresFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
    ...
    double sqrt_info = istd / GNSS_SCALE;
    double r1, e1[3];
...
bool SppCarrierPhaseCeresFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
    const double* xyz = parameters[0];
    const double* dtgps = parameters[1];
...
bool FixedIntegerCeresFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
...
static double varerr2(double el, double dt, double mea_var) {
...
bool RTKCarrierPhaseCeresFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
    const double* xyz = parameters[0];
    ...
    r1 = distance(xyzglobal, satelite_pos, e1);
    double sqrt_info = 1;
    if (use_istd)sqrt_info = 1 / sqrt(varerr2(el, base_rover_time_diff, mea_var)) / GNSS_SCALE;
    if (residuals)
...
bool RTKPseudorangeCeresFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
    const double* xyz = parameters[0];
    ...
    double r1, e1[3];
    r1 = distance(xyzglobal, satelite_pos, e1);
    double sqrt_info = 1 / sqrt(varerr2(el, base_rover_time_diff, mea_var)) / GNSS_SCALE / 2.5;
    if (residuals)
...
bool SppDopplerCeresFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
